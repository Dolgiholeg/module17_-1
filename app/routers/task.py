...
from app.routers.schemas import CreateUser, CreateTask, UpdateTask, UpdateUser
...
async def create_task(db: Annotated[Session, Depends(get_db)], create_task: CreateTask):
    ...
    db.execute(insert(Task).values(title=create_task.title,content=create_task.content,user_id=create_task.user_id,
...
async def update_task(db: Annotated[Session, Depends(get_db)], user_id: int, update_task: UpdateTask):
    ...
    db.execute(update(Task).where(Task.user_id == user_id).values(title=update_task.title,content=update_task.content,
    user_id=update_task.user_id,slug=slugify(update_task.title)))
    db.commit()
